...
describe("CryptoExchange Contract", function () {
  ...
  it("Should allow users to deposit tokens into the exchange", async function () {
    ...
    await token
      .connect(user1)
      .approve(await exchange.getAddress(), ethers.parseUnits("100", 18));

    // Deposit tokens into exchange
    await expect(
      exchange
        .connect(user1)
        .deposit(await token.getAddress(), ethers.parseUnits("100", 18))
    )
    ...
    expect(
      await exchange.balances(user1.address, await token.getAddress())
  ...
  it("Should allow users to transfer tokens within the exchange", async function () {
    ...
    await token
      .connect(user1)
      .approve(await exchange.getAddress(), ethers.parseUnits("50", 18));
    await exchange
      .connect(user1)
      .deposit(await token.getAddress(), ethers.parseUnits("50", 18));
    ...
    expect(
      await exchange.balances(user2.address, await token.getAddress())
  ...
  it("Should allow users to trade tokens within the exchange", async function () {
    ...
    expect(
      await exchange.balances(user1.address, await token.getAddress())
    ).to.equal(ethers.parseUnits("75", 18));
  });
  ...
  it("Should allow users to withdraw tokens from the exchange", async function () {
    ...
    await expect(
      exchange
        .connect(user1)
        .withdraw(await token.getAddress(), ethers.parseUnits("50", 18))
  ...
  it("Should not allow users to withdraw more than their balance", async function () {
    await expect(
      exchange
        .connect(user1)
        .withdraw(await token.getAddress(), ethers.parseUnits("5000", 18))
    ).to.be.revertedWith("Insufficient balance");
  });
});
